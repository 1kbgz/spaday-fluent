...
def write_index(output: Path) -> None:
    imports_file = output / "components" / "fluent" / "vendor" / "imports.json"
    imports = {specifier: f"./components/fluent/{target}" for specifier, target in json.loads(imports_file.read_text(encoding="utf-8")).items()}
