...
await micropip.install(wheels["fluent"], deps=False)

example = importlib.import_module(f"spaday_fluent.{example_name}")
...
async def call_endpoint(path, body_json):
    ...
    if path == "/api/tasks":
        response = await example.create_task(LocalRequest(body))
    elif match := re.fullmatch(r"/api/tasks/([^/]+)/complete", path):
        response = await example.complete_task(LocalRequest(path_params={"id": match.group(1)}))
    else:
...
if hasattr(example, "run_sprint"):
    asyncio.create_task(example.run_sprint())
