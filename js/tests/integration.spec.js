...
/* spaday-fluent authored from Python, on a page it shares with a downstream library that imports
 * Fluent by its bare specifiers. See spaday_fluent/tests/integration.py.
 */

const PAGE = "http://127.0.0.1:8019";
...
test("a library importing Fluent by name gets the page's copy", async ({
  page,
}) => {
  const errors = [];
  ...
  await expect(downstream.locator("fluent-button")).toBeAttached();
