...
.PHONY: catalog
catalog:  ## regenerate the typed catalog from Fluent's manifest
	cd js; pnpm manifest
	python -m spaday.cem spaday_fluent/custom-elements.json -o spaday_fluent/components.py
	python -m ruff format spaday_fluent/components.py
...
lint-py:  ## run python linter with ruff
	python -m ruff check spaday_fluent js/examples/build_pyodide_example.py
	python -m ruff format --check spaday_fluent js/examples/build_pyodide_example.py
...
fix-py:  ## fix python formatting with ruff
	python -m ruff check --fix spaday_fluent
	python -m ruff format spaday_fluent
...
test-py:  ## run python tests
	python -m pytest -v spaday_fluent/tests

# alias
tests-py: test-py

coverage-py:  ## run python tests and collect test coverage
	python -m pytest -v spaday_fluent/tests --cov=spaday_fluent --cov-report term-missing --cov-report xml
...
pyodide-example: build  ## build the standalone Pyodide example into dist/lite
	test -n "$(firstword $(wildcard dist/spaday_fluent-*.whl))"
	...
	python js/examples/build_pyodide_example.py dist/lite "$(firstword $(wildcard dist/spaday_fluent-*.whl))" dist/pyodide-deps
...
test-pyodide-example: pyodide-example  ## run the standalone Pyodide example in Chromium
	...
	cd js; SPADAY_FLUENT_PYODIDE_ONLY=1 pnpm exec playwright test tests/pyodide.spec.js
